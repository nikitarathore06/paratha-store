    ...
    <Box sx={{ minHeight: "10vh", backgroundColor: "#3f98e8" }}>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth>
        ...
        <DialogContent>
          <Stack gap={2}>
            {props.cartItems.map((item) => {
              return <CartItem item = {item} removeFromCart = {props.removeFromCart} />;
            })}
            <Typography style={{ color: "#24c148", fontWeight: "bold", fontSize: "15px", alignSelf: 'flex-end'}}>Total Amount: Rs. {totalPrice}</Typography>
          </Stack>
        </DialogContent>
      </Dialog>
    </Box>
